...
function ProfileLeftbar() {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/user/post/user/delete/${user.other._id}`)
        setUser(res.data);
      } catch (error) {
        console.log(error)
    ...
    const getFollowing = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/user/following/${user.other._id}`
        );
  ...
  const handleFollowing = async (itemId) => {
      await fetch(`http://localhost:5000/api/user/following/${itemId}`, {
        method: "PUT",
        headers: {
  ...
  const yesDelete = async() => {
    try {
      ...
      axios.delete(`http://localhost:5000/api/user/delete/${user.other._id}`)
        .then(res => {
